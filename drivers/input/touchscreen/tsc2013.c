...
//printk(KERN_WARNING "Input coords: %d, %d\n", ts->finger_pos_x[0], ts->finger_pos_y[0]);
				...
				if (calibration[6] == 0) {
					...
					calibration[4] * (int) ts->finger_pos_y[0] +
					calibration[5];
					y /= calibration[6];
					if (y < 0)
						y = 0;
...
//printk(KERN_WARNING "Output coords: %ld, %ld\n", x, y);
#endif

				xin = ts->finger_pos_y[0];
				yin =  ts->finger_pos_x[0];

//printk(KERN_WARNING "Input coords: %ld, %ld\n", xin, yin);

				xout = ( XMAX * ( xin - min_x ) ) / (max_x - min_x);
				yout = ( YMAX * ( yin - min_y ) ) / (max_y - min_y);
				if(xout < 0) xout = 0;
				if(xout > XMAX) xout = XMAX;
				if(yout < 0) yout = 0;
				if(yout > YMAX) yout = YMAX;

//printk(KERN_WARNING "Output coords: %ld, %ld\n", xout, yout);

            			input_report_abs(ts->input, ABS_MT_TRACKING_ID, 0);
				input_report_abs(ts->input, ABS_MT_POSITION_X, xout);
		    		input_report_abs(ts->input, ABS_MT_POSITION_Y, yout);
...
//printk(KERN_WARNING "Multitouch\n");
			  	...
			  	for (i = 0; i < 2; i++)
			  	{
 					...
 					if (calibration[6] == 0) {
                                        	...
                                        	x = calibration[0] * (int)ts->finger_pos_x[i] +
                                        	calibration[1] * (int) ts->finger_pos_y[i] +
                                        	calibration[2];
                                        	x /= calibration[6];
                                        	if (x < 0)
                                        	        x = 0;
                                        	y = calibration[3] * (int) ts->finger_pos_x[i] +
                                        	calibration[4] * (int) ts->finger_pos_y[i] +
                                        	calibration[5];
                                        	y /= calibration[6];
                                        	if (y < 0)
                                        	        y = 0;
